...
main <- function(date = NULL, table = "TestSearchSatisfaction2_15357244"){
  ...
  thresshold_passing_rate <- data.frame(date = date, threshold_pass = sum(dwell_data)/length(dwell_data))
  
  # Write out the results
  wmf::write_conditional(page_visit_survivorship, file.path(base_path, "sample_page_visit_ld.tsv"))
  wmf::write_conditional(thresshold_passing_rate, file = file.path(base_path, "search_threshold_pass_rate.tsv"))
