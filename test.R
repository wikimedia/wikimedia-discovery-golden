...
message("# Test Run\n\n## Parameters")
print(kable(dplyr::bind_cols(opt), format = "markdown"))
...
for (i in 1:nrow(reports)) {
  ...
  if (opt$include_samples) {
    if (grepl("forecasts", reports$module[i], fixed = TRUE)) {
      ...
      filename <- file.path(sub("metrics/", "/a/aggregate-datasets/discovery/", reports$module[i]), paste0(reports$report[i], ".tsv"))
